...
# 为了兼容性，保留原有的CourtSession类
class CourtSession:
    def __init__(self):
        self.context_manager = ContextManager("default_session")
        self.judge = JudgeAgent(self.context_manager)
    ...
    def setup_teams(self, player_role: str):
        if player_role == "plaintiff":
            self.plaintiff_team = PlaintiffAgent(self.context_manager)
            self.defendant_team = DefendantAgent(self.context_manager)
        else:
            self.plaintiff_team = PlaintiffAgent(self.context_manager)
            self.defendant_team = DefendantAgent(self.context_manager)
    ...
    def submit_evidence(self, role: str, evidence: Evidence) -> str:
        ...
        if role == "plaintiff":
            return self.plaintiff_team.present_evidence(evidence)
        else:
            return self.defendant_team.cross_examine_evidence(evidence)

    def advance_trial(self) -> Dict:
        # 使用CourtCoordinator的逻辑
        coordinator = CourtCoordinator("default_session")
        coordinator.context_manager = self.context_manager
        coordinator.judge = self.judge
        coordinator.plaintiff = self.plaintiff_team
        coordinator.defendant = self.defendant_team
        coordinator.current_phase = self.current_phase
        
        response = coordinator.advance_trial()
        self.current_phase = coordinator.current_phase
        return response
